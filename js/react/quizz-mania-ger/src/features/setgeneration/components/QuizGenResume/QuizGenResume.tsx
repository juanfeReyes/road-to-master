...
export const QuizGenResume = ({navigation, onClose}: QuizGenResumeProps) => {
    ...
    return (<div className="flex flex-col gap-4">
        {setup && <>
            {setup.timer && <div>{setup.timer} Minutes</div>}
            <div className="flex justify-end py-2 pr-4 gap-1.5">
                <p>Total questions</p> <p className="font-black">{setup.totalQuestions}</p></div>
            <table className="w-full border-separate">
                ...
                {Object.entries(setup.questResume)
                    .map(([key, value]) => <tr className="text-center hover:bg-slate-100">
